            ...
            using (SqlConnection connection = new SqlConnection(_connectionString))
            {
                ...
                try
                {
                    using (SqlCommand cd = new SqlCommand("sp_CrearCompra", connection))
                    {
                        ...
                        cd.Parameters.AddWithValue("@NombreProveedor", compra.NombreProveedor);
                        cd.Parameters.AddWithValue("@NombreUsuario", compra.NombreUsuario);
                ...
                using (var command = new SqlCommand("SELECT * FROM Compras WHERE ID_Compra = @Id", connection))
                {
                    ...
                    using (var reader = command.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            ...
                            compra.NombreProveedor = reader.GetInt32(1);
                            compra.NombreUsuario = reader.GetInt32(2);
